...
use {
	...
	core::convert::{Infallible, TryFrom},
...
impl<'de, R: dec::Read<'de>> Deserializer<R> {
	...
	fn try_step<'a>(
		...
		scopeguard::ScopeGuard<&'a mut Self, fn(&'a mut Self) -> ()>,
		DecodeError<R::Error>,
	> {
	...
	/// This method should be called after a value has been deserialized to ensure
	/// there is no trailing data in the input source.
...
impl<'de, 'a, R: dec::Read<'de>> serde::Deserializer<'de>
	for &'a mut Deserializer<R>
{
		...
		V: Visitor<'de>,
	{
		...
		match dec::if_major(byte) {
			...
			major::NEGATIVE => {
				// CBOR supports negative integers up to -2^64 which is less than
				// i64::MIN. Only treat it as i128, if it is outside the i64 range.
		...
		V: Visitor<'de>,
	{
		...
		// This is a bit wasteful when encountering strings of more than one
		// character, but we are optimistic this is a cold path.
...
///
/// This is without the CBOR tag information. It is only the CBOR byte string
/// identifier (major type 2), the number of bytes, and a null byte prefixed
/// CID.
///
/// The reason for not including the CBOR tag information is the [`Value`]
/// implementation. That one starts to parse the bytes, before we could
/// interfere. If the data only includes a CID, we are parsing over the tag to
/// determine whether it is a CID or not and go from there.
struct CidDeserializer<'a, R>(&'a mut Deserializer<R>);

impl<'de, 'a, R: dec::Read<'de>> de::Deserializer<'de>
	for &'a mut CidDeserializer<'a, R>
{
	type Error = DecodeError<R::Error>;
	...
	fn deserialize_newtype_struct<V: de::Visitor<'de>>(
		...
		if name == CID_SERDE_PRIVATE_IDENTIFIER {
			...
			Err(de::Error::custom(
				[
					"This deserializer must not be called on newtype structs other than \
					 one named `",
