...
//!
//! This crate defines a `no_std` compatible data structures for representing a
//! `Multihash`.
//!
//! It does not offer any hashing, instead you are encouraged to either do the
//! hashing yourself. Alternatively, you can use an existing code table or make
//! your own code table.
//!
//! The [`multihash-codetable`] crate defines a set of hashes to get started
//! quickly. To make your own codetable, use the [`multihash-derive`] crate.
//!
	...
	W: io::Write,
{
	...
	w.write_all(code).map_err(error::io_to_multihash_error)?;
	w.write_all(size).map_err(error::io_to_multihash_error)?;
	w.write_all(digest).map_err(error::io_to_multihash_error)?;
	...
	R: io::Read,
{
	...
	r.read_exact(&mut digest[..size as usize])
		.map_err(error::io_to_multihash_error)?;
	...
	for i in 0..b.len() {
		...
		if n == 0 {
			...
			return decode::u64(&b[..=i])
				...
				.map_err(error::varint_decode_to_multihash_error);
		}
	}
