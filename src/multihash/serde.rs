...
/// The maximum serialization size of `code` is 9 bytes (a large varint encoded
/// u64) and for `size` is 2 bytes  (a large varint encoded u8), this makes a
/// total of 11 bytes.
const MAXIMUM_PREFIX_SIZE: usize = 11;

/// The is currently no way to allocate an array that is some constant size
/// bigger then a given const generic. Once `generic_const_exprs` are a thing,
/// this struct will no longer be needed. Until then we introduce a hack. We
/// allocate a struct, which contains two independent arrays, which can be
/// specified with const generics. We then treat the whole struct as a slice of
/// continuous memory.
#[repr(C, packed)]
...
#[allow(unsafe_code)]
impl<const SIZE_FIRST: usize, const SIZE_SECOND: usize>
	Buffer<SIZE_FIRST, SIZE_SECOND>
{
	fn new() -> Self {
		Self {
			first: [0; SIZE_FIRST],
	...
	fn as_slice(&self) -> &[u8] {
		unsafe {
			slice::from_raw_parts(self as *const _ as _, mem::size_of::<Self>())
		}
	}

	fn as_mut_slice(&mut self) -> &mut [u8] {
		unsafe {
			slice::from_raw_parts_mut(self as *mut _ as _, mem::size_of::<Self>())
		}
	}
}
...
mod tests {
	...
	const DIGEST: [u8; 32] = [
		159, 228, 204, 198, 222, 22, 114, 79, 58, 48, 199, 232, 242, 84, 243, 198,
		71, 25, 134, 172, 177, 248, 216, 207, 142, 150, 206, 42, 215, 219, 231,
		251,
	];
	...
	fn test_serde_json() {
		...
		let expected_json = format!(
			"[{},{},159,228,204,198,222,22,114,79,58,48,199,232,242,84,243,198,71,\
			 25,134,172,177,248,216,207,142,150,206,42,215,219,231,251]",
			SHA2_256_CODE as u8,
			DIGEST.len() as u8
		);
	...
	fn test_serde_test() {
		...
		// As sequence.
		serde_test::assert_de_tokens(&mh, &[
	...
	fn test_buffer_alignment() {
		...
		// Make sure that the struct allocated continuous memory, as we exploit that
		// fact with the `as_slice` and `as_mut_slice()` methods.
	...
	fn test_buffer() {
		...
		let data: [u8; SIZE_FIRST + SIZE_SECOND] =
